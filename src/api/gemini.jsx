...
export default (
  props,
  { context = undefined, allowPaste = false, useSelected = false, useClipboard = false, clipboardPrefillOnly = false, buffer = [] },
) => {
  const Pages = {
    Form: 0,
    Detail: 1,
    ...
    (async () => {
      if (useSelected) {
        ...
        try {
          ...
          await showToast({
            style: Toast.Style.Failure,
            title: "Could not read the clipboard. Continue without it.",
          });
          if (!clipboardPrefillOnly) {
            getResponse(argQuery);
          } else {
    ...
    <Form
      actions={
        <ActionPanel>
          ...
          <Action
            icon={Icon.Clipboard}
            title="Append Clipboard Text"
            onAction={async () => {
              try {
                const clip = await Clipboard.readText();
                setTextarea((text) => text + (clip ?? ""));
              } catch {
                await showToast({
